...
# Each step is guarded rather than left to errexit: `set -e` is disabled inside
# a function called as an `if` condition, so a failed checkout would otherwise
# fall through to the `reset --hard` on whatever branch we are still on.
sync_default_branch() {
  ...
  if [ -n "$(git status --porcelain --untracked-files=no)" ]; then
    ...
    git stash push -m "sync-target-repo.sh $(date -u +%Y-%m-%dT%H:%M:%SZ)" || return 1
  fi

  git checkout "$BRANCH" || return 1
  git fetch upstream || return 1
  git reset --hard "upstream/$BRANCH" || return 1
}
