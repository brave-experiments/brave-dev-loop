...
- **[docs/pr-descriptions.md](../docs/pr-descriptions.md)** - How a PR body must read for a human reviewer: reproduction first, problem second. Required before writing any PR body; `scripts/check-pr-body.py --diff-base <ref>` enforces it, including that a change to the interface shows the screen it produces
